...
class ArrangePaginatedData:
    'this is just a layer that adds pagination to a view set easyly'
    paginator = StandardResultsSetPagination()


    def get_paginated_data(self,data):
        result_page = self.paginator.paginate_queryset(data, self.request)
        serialized_data= self.serializer_class(result_page,many=True,context={'request':self.request})
        data= self.paginator.get_paginated_response(serialized_data.data).__dict__.get('data')
        return data